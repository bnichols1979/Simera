...
***********************************************************************/
int64_t StochasticLib2::Hypergeometric (int64_t n, int64_t m, int64_t N) {
/*
...
int64_t StochasticLib2::HypPatchwork (int64_t n, int64_t m, int64_t N) {
/* 
...
*/
  ...
  if (N != hyp_N_last || m != hyp_m_last || n != hyp_n_last) { 
    ...
    mode  = (int64_t)modef;
    Hyp_k2 = (int64_t)ceil(modef - 0.5 - U);  
    if (Hyp_k2 >= mode) Hyp_k2 = mode - 1;
    Hyp_k4 = (int64_t)(modef - 0.5 + U);
    ...
    while (1) {
      ...
      if ((U = Random() * Hyp_p6) < Hyp_p2) {                  // centre left
        ...
        // immediate acceptance region R2 = [k2, mode) *[0, f2),  X = k2, ... mode -1
        if ((W = U - Hyp_p1) < 0.)  return(Hyp_k2 + (int64_t)(U/Hyp_f2));
        // immediate acceptance region R1 = [k1, k2)*[0, f1),  X = k1, ... k2-1
        if ((Y = W / Hyp_dl) < Hyp_f1)  return(Hyp_k1 + (int64_t)(W/Hyp_f1));
        ...
        // immediate acceptance region R3 = [mode, k4+1)*[0, f4), X = mode, ... k4
        if ((W = U - Hyp_p3) < 0.)  return(Hyp_k4 - (int64_t)((U - Hyp_p2)/Hyp_f4));
        
        // immediate acceptance region R4 = [k4+1, k5+1)*[0, f5)
        if ((Y = W / Hyp_dr) < Hyp_f5)  return(Hyp_k5 - (int64_t)(W/Hyp_f5));

        // computation of candidate X > k4, and its reflected counterpart V < k4
        // either squeeze-acceptance of X or acceptance-rejection of V
        Dk = (int64_t)(Hyp_dr * Random()) + 1;
